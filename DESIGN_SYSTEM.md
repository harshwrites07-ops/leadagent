...
**Changelog — god-level redesign, Phase 0 (2026-07-02):** Locked the foundation for the full redesign pass. (1) Added strict type-scale tokens (`--fs-xs…--fs-4xl` = 12/14/16/20/24/32/48/64 — nothing between) and 4px-grid spacing tokens (`--sp-1…--sp-16`) to `frontend/src/index.css`. (2) Added the bento grid law: `.bento` (4-col) with exactly three tile sizes — `.bento__1x1`, `.bento__2x1`, `.bento__2x2` — plus card elevation variants `.card--raised` (surface-2) and `.card--hero` (surface-2 + lime wash, max one per screen). (3) Created `frontend/src/lib/motion.js` — the single motion config: `springMicro` (400/30) for micro-interactions, `easeMacro` (200ms) for page/section moves, plus `pageVariants`, `fadeUp`, `stagger`, `springIn`, `rowIn`, `slideInRight`, `modalIn`, `drawPath`. No inline one-off transition values from here on. (4) Rebuilt the core primitives in `frontend/src/components/ui/` on top of the CSS system: `<Button>` (5 variants, `loading` prop with spinner), `<Card>` (default/raised/hero), `<Badge>` (7 kinds, pulse dot), new `<Input>` (label+error+hint), new `<CountUp>` (every data number counts up, monospace, reduced-motion aware), new `<Stat>` (KPI tile = label + CountUp + delta + Sparkline), promoted `<Sparkline>` (path-draw on mount). (5) Stray-color kill: every hex literal in `components/ui/` (except AssistantChat, which Phase 2 replaces with the Agent slide-out) swapped for tokens — PowerSendOverlay's cyan/orange/green rainbow is now sky/warn/ok, `--gradient-orange` uses eliminated in ui/. (6) Added the missing `prefers-reduced-motion` guard to app CSS. Remaining page-level hex strays (Landing 87, Login/Signup 8 ea., Admin 8, CRM 6, etc.) are killed in their phase passes.

**Changelog — god-level redesign, Phases 1-4 (2026-07-02):** Full pass across landing, shell, and core app screens, built on the Phase 0 foundation above.
- **Phase 1 (Landing):** `frontend/src/pages/Landing.jsx` rebuilt from scratch — the live Marcus generation loop (intel flicker → staged terminal status → typewriter email → quality-gate score count-up + lime pulse, ~13.5s seamless loop, static first paint via `prefers-reduced-motion` + `useInView` gating), kinetic rotating headline (client/video editor gig/thumbnail contract/retainer), one lime gradient mesh behind the hero only, Find→Write→Send→Close scroll narrative with product-real visuals, count-up stat band, testimonials, pricing. Mobile nav collapses to a single "Get started" pill.
- **Phase 2 (Shell):** Sidebar nav regrouped into Find/Write/Send/Close/Measure sections (`frontend/src/components/ui/Layout.jsx`), active state now lime left-edge + raised surface. Topbar search replaced with a `.cmdk-trigger` ⌘K-style button. The floating-bot AssistantChat FAB is gone — Agent is now a fixed right-edge slide-out panel (peer, not a support widget), toggled by ⌘K/Ctrl+K, sidebar "Ask Marcus", or the topbar Agent button. "Jack" renamed to "Marcus" everywhere user-visible (internal `jack:open` event/localStorage keys kept for continuity).
- **Phase 3 (Pages):** Dashboard hero pipeline card promoted to `--surface-2` + lime-border elevation with cream serif value and a real count-up; secondary tiles show an honest "trend appears after 7 days" state instead of fabricated bar-chart placeholders. PitchGen's generating state replaced with a terminal-style panel showing the 4 real pipeline stages as staged status lines; the quality-tier badge now spring-scales in on result. CRM cards get a slight rotate + spring scale while dragging. Analytics dedupes its local Sparkline for the shared `<Sparkline>` and the funnel's "No funnel data yet" text upgraded to the standard `.empty` pattern.
- **Phase 4 (Details):** Toasts confirmed already on-token (lime/coral iconThemes); toast/PowerFollowUpOverlay's last literal border color moved to `var(--coral-border)`. Added `/logo.svg` (lime/coral split-square, was 404 — no favicon existed before this pass) + OG/Twitter meta tags to `frontend/index.html`. Full stray-hex sweep across every page (`grep -r "#[0-9a-fA-F]\{6\}" frontend/src` now returns only the Google G-logo brand colors in Login/Signup and the Razorpay checkout `theme.color`, both legitimate third-party literals). Remaining `linear-gradient` uses are all on decorative avatars/logos/skeleton shimmer, never on `.btn`.

**Known gaps for a future pass:** Settings/Campaigns/Onboarding/auth pages got the mechanical hex sweep but not a full visual rebuild (no bento grid, no count-up numbers yet). QualityLeads' terminal aesthetic was left as-is per the brief ("bring into the system last"). OG image is currently the SVG mark, not a rendered 1200×630 PNG — most link unfurlers accept SVG today but a raster fallback would be more compatible.
