...
/* ──────────────────────────────────────────────────────
   Quelro — outreach OS for freelancers selling to YouTube creators
...
/* ─────────────────────────────────────────────
   Card1 – Lead Discovery Pipeline
   Query → Router → Intent Scorer → Leads DB / Enrichment
   ───────────────────────────────────────────── */
...
const NODES: NodeConfig[] = [
  { id: 'A', x: 50, y: 120, icon: ChatCircle, label: "QUERY", type: 'box' },
  { id: 'Router', x: 125, y: 120, type: 'circle' },
  { id: 'C', x: 200, y: 120, icon: Brain, label: "SCORER", type: 'box' },
  { id: 'B', x: 280, y: 50, icon: Database, label: "LEADS DB", type: 'box' },
  { id: 'D', x: 280, y: 190, icon: TerminalWindow, label: "ENRICH", type: 'box' },
];
...
/* ─────────────────────────────────────────────
   Card2 – Signal Monitor (scan volume + vacancy hits)
───────────────────────────────────────────── */
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between">
      ...
      <div className="flex gap-4 pt-[0.625rem] pr-[0.625rem] pb-0.5 pl-0.5">
        {[
          { label: "Channels scanned", value: "1.2k/hr", trend: "+8%" },
          { label: "Vacancy hits", value: "14/hr", trend: "+3%" },
        ].map((s, i) => {
...
/* ─────────────────────────────────────────────
   Card3 – Outreach Pipeline Activity (stacked feed)
   ───────────────────────────────────────────── */
...
export function Card3() {
  const logs = [
    { agent: "Scraper", action: "Scanned new channel descriptions across 4 sources", status: "done", t: "0.2s" },
    { agent: "Intent Scorer", action: "Ranked leads by niche, subs and engagement signals", status: "done", t: "1.4s" },
    { agent: "Credit Diff", action: "Detected editor-slot vacancy — signal logged", status: "running", t: "3.1s" },
    { agent: "Allocation Engine", action: "Checking claim window before surfacing lead", status: "waiting", t: "—" },
    { agent: "Email Writer", action: "Idle — queued for next claimed lead", status: "idle", t: "—" },
  ];
...
/* ─────────────────────────────────────────────
   Card4 – Lead Sources (YouTube, podcasts, community signals, job boards)
   ───────────────────────────────────────────── */
...
const NS_COLORS: Record<string, { bar: string; dot: string; badge: string; buttonBg: string; buttonBorder: string }> = {
  youtube: { bar: "from-violet-500 to-violet-400", dot: "bg-violet-500", badge: "bg-violet-500/15 text-violet-400", buttonBg: "bg-violet-500", buttonBorder: "border-violet-600" },
  podcasts: { bar: "from-sky-500    to-sky-400", dot: "bg-sky-500", badge: "bg-sky-500/15 text-sky-400", buttonBg: "bg-sky-500", buttonBorder: "border-sky-600" },
  community: { bar: "from-emerald-500 to-emerald-400", dot: "bg-emerald-500", badge: "bg-emerald-500/15 text-emerald-400", buttonBg: "bg-emerald-500", buttonBorder: "border-emerald-600" },
  jobs: { bar: "from-amber-500  to-amber-400", dot: "bg-amber-500", badge: "bg-amber-500/15 text-amber-400", buttonBg: "bg-amber-500", buttonBorder: "border-amber-600" },
};

const RETRIEVAL_QUERIES = [
  { ns: "youtube", q: "editor-slot vacancy — credit diff on channel", t: "0.2s" },
  { ns: "podcasts", q: "new show detected, no ad-read cadence yet", t: "1.1s" },
  { ns: "youtube", q: "subs crossed 20K — sweet-spot tier match", t: "2.4s" },
  { ns: "community", q: "r/NewTubers — creator asking for editor recs", t: "4.0s" },
  { ns: "jobs", q: "freelance thumbnail gig posted, matches niche", t: "5.8s" },
  { ns: "podcasts", q: "self-edited channel — DIY creator signal", t: "7.2s" },
];

export function Card4() {
  const namespaces = [
    { name: "youtube", hits: 342, fill: 88 },
    { name: "podcasts", hits: 218, fill: 56 },
    { name: "community", hits: 97, fill: 25 },
    { name: "jobs", hits: 54, fill: 14 },
  ];
  ...
  return (
    <div className="w-full h-full flex gap-5 py-2 px-3">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 pr-2">
        <p className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-3">Lead sources</p>
        ...
        <div className="flex items-center gap-2 pt-3 mt-auto">
          ...
          <span className="text-[8px] font-mono text-muted-foreground font-medium tracking-wide">Scraper loop active</span>
        </div>
      </div>
      ...
      <div className="w-[172px] shrink-0 flex flex-col gap-0">
        <p className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-2.5">Signal log</p>
...
/* ─────────────────────────────────────────────
   Card5 – Signal Detectors (channel scan, credit diff, email verify, intent score)
───────────────────────────────────────────── */
export function Card5() {
  const tools = [
    { name: "channel_scan", calls: 14, icon: Globe, latency: "280ms", color: "bg-gradient-to-b from-sky-400 to-sky-600", borderColor: "border-sky-600" },
    { name: "credit_diff", calls: 8, icon: TerminalWindow, latency: "1.2s", color: "bg-gradient-to-b from-emerald-400 to-emerald-600", borderColor: "border-emerald-600" },
    { name: "email_verify", calls: 22, icon: FileText, latency: "12ms", color: "bg-gradient-to-b from-amber-400 to-amber-600", borderColor: "border-amber-600" },
    { name: "intent_score", calls: 31, icon: Brain, latency: "95ms", color: "bg-gradient-to-b from-violet-400 to-violet-600", borderColor: "border-violet-600" },
  ];
...
const CARDS = [
  {
    title: "Lead Discovery Pipeline",
    description: "Watch a query flow from source scan to a scored, allocation-ready lead in real time.",
    visual: <Card1 />,
  ...
  },
  {
    ...
    description: "Track channels scanned and vacancy-signal hits across every scraper run.",
    visual: <Card2 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  },
  {
    title: "Outreach Pipeline Activity",
    description: "Live log of every stage — scrape, score, credit-diff, allocate, and send-ready drafts.",
    visual: <Card3 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  },
  {
    title: "Lead Sources",
    description: "YouTube, podcasts, community signals, and job boards — indexed into one searchable pool.",
    visual: <Card4 />,
  ...
  },
  {
    ...
    description: "Channel scans, credit diffs, email verification, and intent scoring running behind every lead.",
    visual: <Card5 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  }
