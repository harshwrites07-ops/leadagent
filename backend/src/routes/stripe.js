...
router.get('/subscription', asyncHandler(async (req, res) => {
  const user = req.user;
  const db = getDb();
  const fresh = db.prepare(`SELECT plan, plan_status, stripe_customer_id, stripe_subscription_id, trial_ends_at, leads_used_this_month, emails_used_this_month, usage_reset_date FROM users WHERE id=?`).get(user.id);
  ...
  res.json({
    ...
    has_billing: !!fresh.stripe_customer_id,
    usage: {
...
webhookRouter.post('/', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  function applySubscription(sub) {
    ...
    db.prepare(`UPDATE users SET plan=?, plan_status=?, stripe_subscription_id=?, billing_cycle_start=? WHERE id=?`)
      .run(plan, status, sub.id, new Date(sub.current_period_start * 1000).toISOString(), userId);
    console.log(`[Stripe] User ${userId} → plan=${plan} status=${status}`);
  }
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (session.subscription) {
        stripe.subscriptions.retrieve(session.subscription).then(applySubscription).catch(console.error);
      }
      break;
    }
    ...
    case 'customer.subscription.deleted': {
      ...
      const userId = sub.metadata?.user_id;
      if (userId) {
        db.prepare(`UPDATE users SET plan='free', plan_status='cancelled', stripe_subscription_id=NULL WHERE id=?`).run(userId);
        console.log(`[Stripe] User ${userId} subscription cancelled → plan=free`);
      }
      break;
    }
    case 'invoice.payment_failed': {
      ...
      if (invoice.subscription) {
        stripe.subscriptions.retrieve(invoice.subscription).then(sub => {
          const userId = sub.metadata?.user_id;
          if (userId) {
            db.prepare(`UPDATE users SET plan_status='past_due' WHERE id=?`).run(userId);
            console.log(`[Stripe] User ${userId} payment failed → past_due`);
          }
        }).catch(console.error);
      }
      break;
