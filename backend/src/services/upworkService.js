...
const BASE_URL = 'https://www.upwork.com/ab/feed/jobs/rss';

const CREATOR_KEYWORDS = [
  'youtube', 'youtuber', 'channel', 'subscriber', 'views',
  'video editor', 'thumbnail', 'scriptwriter', 'content creator',
  'editing', 'creator', 'vlogger', 'streamer'
];
...
function fetchRSS(query) {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      path: `/ab/feed/jobs/rss?q=${query}&sort=recency&paging=0%3B20`,
      headers: {
...
function parseRSSItems(xml) {
  ...
  for (const item of itemMatches) {
    ...
    const isRelevant = CREATOR_KEYWORDS.some(kw => fullText.includes(kw));
    ...
    items.push({
      ...
      jobUrl: link,
      postedAt: pubDate,
      confidence: calculateJobConfidence(fullText),
    });
  }
...
async function scanUpworkJobs() {
      ...
      for (const item of items) {
        try {
          const result = db.prepare(`
            INSERT OR IGNORE INTO upwork_signals
            (job_id, title, description, budget, job_url, email, posted_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `).run(
            item.jobId,
            item.title,
          ...
          if (result.changes > 0) {
            ...
            db.prepare(`
              ...
              (creator_id, platform, signal_type, signal_text, signal_url, confidence, budget_mentioned)
              VALUES (?, 'upwork', 'confirmed_hiring', ?, ?, ?, ?)
            `).run(
              `upwork_${item.jobId}`,
              item.title,
              item.jobUrl,
            ...
            db.prepare(`
              ...
              VALUES ('upwork', ?, 'upwork_jobs', ?, 'CONFIRMED_HIRING')
            `).run(
              item.jobId,
  ...
  return {
    ...
    new_jobs: totalNew,
    queries_scanned: UPWORK_QUERIES.length,
  };
}
...
async function getUpworkStats() {
  ...
  const total = db.prepare('SELECT COUNT(*) as count FROM upwork_signals').get();
  const today = db.prepare(`
    SELECT COUNT(*) as count FROM upwork_signals
    WHERE date(found_at) = date('now')
  `).get();
  const withBudget = db.prepare(`
    SELECT COUNT(*) as count FROM upwork_signals WHERE budget IS NOT NULL
  `).get();
  const recent = db.prepare(`
    SELECT title, budget, job_url, found_at
    FROM upwork_signals
  ...
  return {
    ...
    found_today: today.count,
    with_budget: withBudget.count,
    recent_jobs: recent,
  };
}
